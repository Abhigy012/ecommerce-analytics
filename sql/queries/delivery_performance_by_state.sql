-- Delivery performance by customer state: actual vs. estimated delivery
-- time, and on-time rate.
...
SELECT
    ...
    ROUND(AVG(julianday(o.order_estimated_delivery_date)
             - julianday(o.order_delivered_customer_date)), 2)          AS avg_days_early_vs_estimate,
    ROUND(100.0 * SUM(CASE WHEN o.order_delivered_customer_date
                                 <= o.order_estimated_delivery_date
                            THEN 1 ELSE 0 END) / COUNT(*), 1)           AS pct_on_time
FROM orders o
